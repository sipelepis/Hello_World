...
class _BreadthFirstPageState extends State<BreadthFirstPage> with SingleTickerProviderStateMixin {
  TreeNode? _root;
  String input = '';
  ...
  void bfsTraversalFunction() {
    ...
    while (queue.isNotEmpty) {
      ...
      bfsTraversal.add(node.index); // Store the index of the node
  ...
  Future<void> animateTraversal() async {
    ...
    for (int i = 0; i < bfsTraversal.length; i++) {
      ...
      setState(() {
        highlightedNodes.add(bfsTraversal[i]); // Highlight the current node by index
      });
      Timer(Duration(milliseconds: 500), () {
        setState(() {
          highlightedNodes.remove(bfsTraversal[i]); // Remove highlight after delay
        });
      });
    }
    // Highlight the last node after traversal
    setState(() {
      highlightedNodes.add(bfsTraversal.last);
    });
  }
          ...
          children: [
            ...
            Column(
              children: [
                ...
                SizedBox(height: 16),
                ElevatedButton(
                  onPressed: bfsTraversalFunction,
