...
class TreePainter extends CustomPainter {
  ...
  void _paintNode(
    ...
    TextSpan span = TextSpan(
        style: TextStyle(color: Colors.black), text: node.originalValue);
...
class _BinarySearchPageState extends State<BinarySearchPage>
                  ...
                  onPressed: () async {
                    try {
                      ...
                      await Future.delayed(
                          Duration(seconds: 2)); // Wait for focus to settle.
  ...
  void checkTree() {
    ...
    setState(() {
      ...
      _missingNodes = formatDiscrepancies(
          missingValues, extraValues); // Prepare text for display
    });
  }

  String formatDiscrepancies(Set<int> missing, Set<int> extra) {
    String missingText =
        missing.isEmpty ? '' : 'Missing Nodes: ${missing.join(", ")}';
    String extraText =
        extra.isEmpty ? '' : 'Nodes not in the given: ${extra.join(", ")}';
    return (missingText +
            (missingText.isNotEmpty && extraText.isNotEmpty ? '\n' : '') +
            extraText)
        .trim();
  }
  ...
  Set<int> gatherTreeValues(TreeNode? node) {
    if (node == null) return {};
    return {node.value}
      ..addAll(gatherTreeValues(node.left))
      ..addAll(gatherTreeValues(node.right));
  }
  ...
  void _captureAndRecognizeText() async {
    ...
    TextEditingController givenController =
        TextEditingController(text: lines.first);

    List<List<TextEditingController>> controllersPerLevel = List.generate(
        levelCount,
        (level) => List.generate(pow(2, level).toInt(), (index) {
              String text = '';
              ...
              actions: [
                    ...
                    children: [
                      ...
                      ElevatedButton(
                          onPressed: () {
                            ...
                            List<TextEditingController> newLevelControllers =
                                List.generate(pow(2, newLevelIndex).toInt(),
                                    (_) => TextEditingController(),
                                    growable: false);
                            setState(() =>
                                controllersPerLevel.add(newLevelControllers));
                          },
                ...
                Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                          onPressed: () {
                            List<String> values = givenController.text
                                .split(RegExp(r'[\s,]+'))
  ...
  void extractNodeValues(List<List<TextEditingController>> controllersPerLevel,
    ...
    List<String?> user_nodes = controllersPerLevel
        .expand((level) => level.map(
            (controller) => controller.text.isEmpty ? null : controller.text))
        .toList();

    Set<String> nonNullUserNodes =
        user_nodes.where((node) => node != null).map((node) => node!).toSet();
    Set<String> nonNullGivenNodes =
        _nodeValues.map((value) => value.toString()).toSet();

    bool discrepanciesExist =
        nonNullUserNodes.difference(nonNullGivenNodes).isNotEmpty ||
    ...
    if (discrepanciesExist) {
      ...
      if (nonNullUserNodes.difference(nonNullGivenNodes).isNotEmpty) {
        message +=
            'Extra node(s) in user input: ${nonNullUserNodes.difference(nonNullGivenNodes).join(", ")}.\n';
      }
      if (nonNullGivenNodes.difference(nonNullUserNodes).isNotEmpty) {
        message +=
            'Missing node(s) in user input that are in given: ${nonNullGivenNodes.difference(nonNullUserNodes).join(", ")}.\n';
      }
  ...
  void showOverlayAlert(
      BuildContext context, String message, List<String?> user_nodes) {
    late OverlayEntry
        overlayEntry; // Declare it as late so it can be initialized later

    overlayEntry = OverlayEntry(
        builder: (context) => Positioned(
              top: MediaQuery.of(context).size.height *
                  0.3, // Positioned in the middle of the screen vertically
              left: 20,
              right: 20,
              child: Material(
                elevation: 5.0,
                child: Container(
                  ...
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Text('Node Discrepancy Detected',
                          style: TextStyle(
                        ...
                        children: [
                          ElevatedButton(
                            onPressed: () {
                              ...
                              buildTree(user_nodes);
                              Navigator.of(context)
                                  .pop(); // Proceed to build the tree despite discrepancies
                            },
                          ...
                          ElevatedButton(
                            onPressed: () {
                              overlayEntry
                                  .remove(); // Close the overlay and allow further editing
                            },
    ...
    Overlay.of(context).insert(
        overlayEntry); // Insert the overlay into the overlay of the context
  }
  ...
  void buildTree(List<String?> user_nodes) {
    ...
    if (user_nodes.isNotEmpty && user_nodes[0] != null) {
      ...
      Queue<TreeNode> queue =
          Queue<TreeNode>(); // Using Queue from dart:collection
      if (_root != null) {
        queue.add(
            _root!); // Safely add _root to the queue after checking it's not null
      }
      ...
      while (queue.isNotEmpty && index < user_nodes.length) {
        ...
        if (index < user_nodes.length && user_nodes[index] != null) {
          currentNode.left = TreeNode(user_nodes[index],
              int.parse(user_nodes[index]!), currentNode.index * 2 + 1);
          queue.add(currentNode
              .left!); // currentNode.left is guaranteed non-null here
        }
        index++;

        // Right child
        if (index < user_nodes.length && user_nodes[index] != null) {
          currentNode.right = TreeNode(user_nodes[index],
              int.parse(user_nodes[index]!), currentNode.index * 2 + 2);
          queue.add(currentNode
              .right!); // currentNode.right is guaranteed non-null here
        }
        index++;
      }
    }
